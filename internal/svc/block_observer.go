...
func newBlkObserver(mgr *Manager, repo *repository.Repository, log *logger.AppLogger) *blkObserver {
	return &blkObserver{
		repo:      repo,
		log:       log,
		mgr:       mgr,
		sigStop:   make(chan bool, 1),
		outEvents: make(chan types.Log, logEventQueueCapacity),
		topics:    nil,
	}
}
